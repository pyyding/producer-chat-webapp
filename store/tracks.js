...
export const actions = {
    ...
    async deleteTrackReply({ commit, dispatch }, id) {
        await fb.db
            .collection('answers')
            .doc(id)
            .delete()
    },
    async getTrackData({ commit, dispatch }, id) {
        const questionSnapshot = await fb.db
            .collection('questions')
            .doc(id)
            .get()
        const answersSnapshot = await fb.db
            .collection('answers')
            .where('questionID', '==', id)
            .get()
        const answers = []
    ...
    async castVote({ commit, dispatch }, { vote, questionID }) {
        ...
        if (existingVote.length === 1) {
            if (existingVote[0].data().isUpvote !== vote.isUpvote) {
                await fb.db
                    .collection('votes')
                    .doc(existingVote[0].id)
                    .delete()
                const newVoteRef = await fb.db
                    .collection('votes')
                    .doc(`${vote.answerID}_${vote.userID}`)
                await newVoteRef.set(vote)
                setTimeout(() => {
                    dispatch('getTrackData', questionID)
                }, 500)
            }
        } else {
            const newVoteRef = await fb.db
